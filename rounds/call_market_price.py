...
class Principle(Enum):
    '''
    Enumeration for the market prinicle used to determine the market price.
...
class MarketPrice:
    ...
    def __init__(self, bids, offers):
        bids, offers = self.ensure_tuples(bids, offers)
        self.bids, self.offers = bids, offers

        self.has_bids = bids != None and len(bids) > 0
        self.has_offers = offers != None and len(offers) > 0
        self.price_df = None
    ...
    def get_cxq(self, price, orders, otype):
        '''
        Determine the cumulative buy/sell quanity for the given price with the give 
        ...
        Parameters
            price (number): The reference price used in filtering the list of oders
            ...
            otype (OrderType): This controls how the list of orders are compared to the
                        reference price.  If the otype == OrderType.BID then concider
    ...
    def ensure_tuple(self, obj):
        if type(obj) == tuple:
            return obj
        else:
            return (obj.price, obj.quantity)

    def ensure_tuples(self, bids, offers):
        #Ensure input data is in the form of price/quant tuples
        if (bids):
            bids = [self.ensure_tuple(o) for o in bids]

        if (offers):
            offers = [self.ensure_tuple(o) for o in offers]
    ...
    def apply_max_volume_princ(self):
        ...
        self.price_df['cbq'] = self.price_df.price.apply(self.get_cxq, args=(self.bids, OrderType.BID))
        self.price_df['csq'] = self.price_df.price.apply(self.get_cxq, args=(self.offers, OrderType.OFFER))
    ...
    def get_market_price(self, last_price = 0):
        ...
        https://www.researchgate.net/publication/241567616_Algorithm_of_Call_Auction_Price
           ...
           Parameters: 
                ...
                debug (Boolean) - when true, the return tuple contains a dataframe and an
                                    enumeration indicating the last applied princile
            
            Returns: A tuple containing
                market_price (number): The resulting market price
                volume (number):  the Market Exchange Volume
                principle (Principle):  Enum reprensting the final principle applied 
                                        to the determine the price.  Useful for testing
                df (DataFrame):  Pandas DataFrame containing theresults of each principle.
        ...
        return self.finalize_and_get_result() # at this point, we are guarenteed one price
...
class OrderFill:
    ...
    def partial_fill(self, order_list, cap):
        """
        ...
        that the total quantity of the return list equals the cap.
            ...
            Returns:
                A list of orders modified in such a way that the combined quanity equals the 
                given cap.
        """
        ret_list = []
    ...
    def fill_orders(self, market_price):
        ...
        Note most 'quantity_final's will be the same as the order quanity.  A few might be different to 
        ...
        total_volume_bid = self.count_volume(bids_to_exchange)
        total_volume_offer = self.count_volume(offers_to_exchange)
        ...
        if total_volume_bid < total_volume_offer:
            ...
            filled_offers = self.partial_fill(offers_to_exchange, mev)
            
        elif total_volume_bid > total_volume_offer:
            #All offers will be traded - bids are modified.
            filled_bids = self.partial_fill(bids_to_exchange, mev)
    ...
    def count_filled_volume(self, orders):
        """
